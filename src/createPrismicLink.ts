...
export type PrismicLinkConfig = Omit<HttpOptions, "fetch" | "useGETForQueries"> &
	(
		| {
				...
				/**
				 * The GraphQL API endpoint for the link's Prismic repository. If a value is not given, the
				 * link will use the default Prismic GraphQL API endpoint for the link's Prismic repository
				 * using the `repositoryName` parameter.
				 */
				uri?: string
		  }
		| {
		...
		/**
		 * The Rest API endpoint for the link's Prismic repository. If a value is not given, the link
		 * will use the default Prismic Rest API endpoint for the link's Prismic repository using the
		 * `repositoryName` parameter.
		 */
		apiEndpoint?: string

		/**
		 * The function used to make network requests to the Prismic API. In environments where a global
		 * `fetch` function does not exist, such as Node.js, this function must be provided.
		 */
		fetch?: FetchLike
	}

/**
 * Creates an Apollo Link that sends GraphQL queries to a Prismic repository's GraphQL API.
 *
 * @example
 * 	```ts
...
export const createPrismicLink = (config: PrismicLinkConfig): ApolloLink => {
	...
	return createHttpLink({
		uri,
		fetch: client.graphQLFetch as typeof globalThis.fetch,
